...
#--------------------------------
input_size = 32 * 32 * 3
hidden_size = [[256], [256,128], [256,128,64], [256,128,64,32], [256,128,64,32,16]]
#hidden_size = [[50], [150,50], [150,50,20], [150,50,30,20], [150,70,50,30,20]]
num_classes = 10
num_epochs = 10
batch_size = 200
learning_rate = 1e-3
...
'''
#################################### IMPORTANT! ##############################################
### We copy paste 2 times the original code for create multiple layers model and plot it. ####
##############################################################################################
train_score =[]
validation_score = []
test_score = []
...
for hidden in hidden_size:
    ...
    validation_score.append(fake_val[-1])
    #ff = fake_val.index(max(fake_val))
    train_score.append(fake_train[-1])
    
    ult_acc_train.append(fake_train)
    ult_acc_val.append(fake_val)
    
    ult_loss_train.append(fake_loss_train)
    ult_loss_val.append(fake_loss_val)
    
#############################################################################################################################
############################################ Start the Model with train = False #############################################
#############################################################################################################################
...
print('Max train score values: ', train_score)
print('Max validation score: ', validation_score)
print('Max test score: ', test_score)
...
plt.title('Epochs vs Accuracy and Loss - Linear + ReLU', size = 25)
for i in range(10):
    plt.subplot(2,5,i+1)
    if i < 5:
        plt.title(f'{i+2} layers', size = 25)
        plt.plot(epochs, ult_acc_train[i], label = 'train', marker = 'o', linewidth = 2)
        plt.plot(epochs, ult_acc_val[i], label = 'validation', marker = 'o', linewidth = 2)
        ...
        plt.plot(epochs, ult_loss_train[i-5], label = 'train', marker = 'o', linewidth = 2)
        plt.plot(epochs, ult_loss_val[i-5], label = 'validation', marker = 'o', linewidth = 2)
        plt.xticks(epochs)
        plt.xlabel('Number of epochs', size = 15)
